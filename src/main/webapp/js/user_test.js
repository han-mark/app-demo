layui.use(['table','jquery','form'], function(){
    ...
    table.render({
        ...
        ,id: 'test'
        ,height: 558
        ,url: ctx + '/findAll' //数据接口,默认会带？page=1,limit=10,返回的数据有格式要求
        ,method: 'post'
        ,even: true //开启隔行背景
        ,page: true //开启分页
        ,cols: [[ //表头
    ...
    table.on('rowDouble(test)', function(obj){
        ...
        layer.open({
            ...
            shadeClose:false,
            closeBtn:1,
            content:ctx + '/pages/updateuser_test.jsp?uuid='+obj.data.uuid + '&flag=1', //flag有值为查看详情页
            end:function(){
    ...
    table.on('tool(test)', function(obj){ //注：tool是工具条事件名，test是table lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            ...
            layer.open({
                ...
                shadeClose:false,
                closeBtn:1,
                content:ctx + '/pages/updateuser_test.jsp?uuid='+data.uuid + '&flag=1', //flag有值为查看详情页
                end:function(){
                    // table.reload('test'); //对应table.render中定义的id
                }
            });
            ...
            layer.open({
                ...
                shadeClose:false,
                closeBtn:1,
                content:ctx + '/pages/updateuser_test.jsp?uuid='+data.uuid,
                end:function(){
                    table.reload('test'); //对应table.render中定义的id
                }
            });
    ...
    $("#add").bind("click",function(){
        ...
        layer.open({
            ...
            shadeClose:false,
            closeBtn:1,
            content:ctx + '/pages/updateuser_test.jsp',
            end:function(){
                table.reload('test'); //对应table.render中定义的id
            }
        });
